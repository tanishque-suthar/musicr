...
-- Sequential extraction: fetch each video's raw streaming URL
    ...
    mp.command_native_async(command, function(success, result)
        if success and result.status == 0 and result.stdout then
            ...
            if title and raw_url then
                ...
                mp.msg.warn("Failed to parse title/URL from output")
            end
        else
            mp.msg.warn("yt-dlp failed for index " .. idx .. ": " .. (result and result.status or "no result"))
        end
        fetch_raw_url(ids_to_fetch, idx + 1)
    end)
end

-- Fetch IDs from YouTube Mix (takes a range like "32-52")
    ...
    mp.command_native_async(command, function(success, result)
        if success and result.status == 0 and result.stdout then
            ...
            if #ids > 0 then
                ...
                mp.msg.warn("No IDs fetched from range " .. start_idx .. "-" .. end_idx)
            end
        else
            mp.msg.warn("Failed to fetch IDs for playlist range " .. start_idx .. "-" .. end_idx)
        end
    end)
end

-- On file loaded: initialize with first track and start fetching mix
mp.register_event("file-loaded", function()
    ...
    if radio_url and not has_loaded then
        ...
        draw_ui()
        -- Start fetching first batch of tracks
        fetch_ids(radio_url, 2, 21)
    end
end)

-- On track change: update current index and fetch more if needed
mp.register_event("start-file", function()
    ...
    -- INFINITE CHECK: If near end of loaded titles, fetch 20 more
